...
const sb = (() => {
  ...
  return {
    async select(table, qs = '') {
      const r = await fetch(u(table, qs), {
        headers: h()
      });
    ...
    async remove(table, id) {
      const r = await fetch(u(table, `?id=eq.${id}`), {
        method: 'DELETE',
        headers: h()
      });
      return r.ok;
...
console.log('rangeHealth input =>', { price, low, high });  
  if (!isFinite(price) || !isFinite(low) || !isFinite(high))
    return {
      label: 'No live price',
      cls: 'warn',
      detail: 'Enter price above to enable range checks.',
      urgency: 1,
      pct: null
    };
  if (price < low) {
    ...
    return {
      label: 'Below range',
      cls: 'out',
      detail: `Price is ${d}% below the lower bound.`,
      urgency: 5,
      pct: 0
    };
  }
  if (price > high) {
    ...
    return {
      label: 'Above range',
      cls: 'out',
      detail: `Price is ${d}% above the upper bound.`,
      urgency: 5,
      pct: 100
    };
  }
  ...
  if (pct <= 12) return {
    label: 'Near lower edge',
    cls: 'warn',
    detail: 'Bot is close to its buy floor.',
    urgency: 3,
    pct
  };
  if (pct >= 88) return {
    ...
    cls: 'warn',
    detail: 'Bot is close to selling out.',
    urgency: 3,
    pct
  };
  return {
    label: 'Healthy',
    cls: 'ok',
    detail: 'Price sits comfortably within the configured range.',
    urgency: 0,
    pct
  };
}
...
// ── Helpers ────────────────────────────────────────────────────
const fmt = (n, d = 2) => isFinite(n) ? n.toLocaleString('en-US', {
...
function getVisible() {
  ...
  let list = state.bots.map(b => ({
    ...b,
    health: rangeHealth(p, b.rangeLow, b.rangeHigh)
  }));
  const sf = el.filterStrategy.value,
    af = el.filterAlert.value;
  ...
  switch (el.sortBy.value) {
    ...
    case 'capital-desc':
      ...
      break;
    case 'alert-first':
      list.sort((a, b) => b.health.urgency - a.health.urgency || b.totalProfit - a.totalProfit);
      break;
    case 'newest':
...
async function submitApiBot() {
  ...
  if (!key || !sec || !pass) {
    showMsg(el.apiMsg, 'All three API fields are required.', 'error');
    return;
  }
...
async function fetchPrice() {
  ...
  const sources = [
    async () => {
        const r = await fetch(proxy(kuLevel1), {
      ...
      async () => {
          const r = await fetch(proxy(kuStats), {
            signal: AbortSignal.timeout(5000)
          });
          const d = await r.json();
        ...
        async () => {
          const r = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=eqty&vs_currencies=usd', {
...
function showMsg(elem, msg, type) {
  ...
  setTimeout(() => {
    elem.textContent = '';
    elem.className = 'submit-msg helper';
  }, 7000);
}
...
function exportJson() {
  const blob = new Blob([JSON.stringify(state.bots, null, 2)], {
...
el.loadSample.addEventListener('click', () => {
  el.botPasteInput.value = SAMPLE_PASTE;
  el.ownerInput.value = '@you';
  el.strategyInput.value = 'A';
});
...
el.botPasteInput.addEventListener('keydown', e => {
  if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') submitPasteBot();
});
